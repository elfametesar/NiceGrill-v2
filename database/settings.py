...
def set_storage_channel(channel_id: str):
    ...
    return Mongo.insert_one({"Storage Channel": channel_id})


def set_ai_proxy(proxy_info: str):
    delete_data("AI Proxy")
    return Mongo.insert_one(({"AI Proxy": proxy_info}))


def set_gpt_chat(chat_id: str):
    delete_data("GPTChat")
    return Mongo.insert_one(({"GPTChat": chat_id}))


def set_ai_behavior(behavior: str):
    delete_data("Gemini Behavior")
    return Mongo.insert_one(({"Gemini Behavior": behavior}))


def set_sticker_pack(sticker_pack: str):
    delete_data("Sticker Pack")
    return Mongo.insert_one(({"Sticker Pack": sticker_pack}))


def set_ai_metadata(metadata: str):
    delete_data("AI Metadata")
    return Mongo.insert_one(({"AI Metadata": metadata}))


def set_download_path(download_path: str):
    delete_data("Download Path")
    return Mongo.insert_one({"Download Path": download_path})


def set_shell_mode(switch: bool | str):
    delete_data("Shell Mode")
    return Mongo.insert_one({"Shell Mode": switch})


def set_prefix(new_prefix: str):
    delete_data("Prefix")
    return Mongo.insert_one({"Prefix": new_prefix})


def set_restart_details(chat_id: int, message_id: int):
    delete_data("Message")
    return Mongo.insert_one({"Chat": chat_id, "Message": message_id})
