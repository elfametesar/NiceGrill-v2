...
class DirectGen:
    ...
    async def direct_link_generator(client: Client, message: Message):
        ...
        url = message.raw_args

        if message.reply_to_text:
            url = message.reply_to_text.raw_text
    ...
    def mega_dl(url: str) -> str:
        ...
        if not os.path.exists("megafetch.sh"):
            os.popen(
                "curl https://gist.githubusercontent.com/zanculmarktum/170b94764bd9a3da31078580ccea8d7e/raw/1adfba71a69ef155c1180ab21e8fb6bead1a6c92/megafetch.sh -o megafetch.sh"
            )

        pattern = f"bash megafetch.sh {link}"
