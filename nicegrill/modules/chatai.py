...
class ChatAI:
    ...
    async def generate_image_from_input(client: Client, message: Message):
        ...
        if not message.raw_args:
            await message.edit(
                "<i>You need to describe the image you want to generate</i>"
            )
            return
        ...
        await message.respond(files=image_list, message="<i>Generated by Perchance</i>")
    ...
    async def converse_with_multi_ai(client: Client, message: Message):
        ...
        data = {
            ...
            "max_tokens": 0,
            "messages": [
                {"role": "system", "content": ChatAI.AI_BEHAVIOR},
                {"role": "user", "content": message.raw_args},
            ],
        }

        response = ChatAI.CLIENT.post(
            url="https://openrouter.ai/api/v1/chat/completions",
            cookies={"__session": OPENROUTER_COOKIE},
...
__{response}__""",
            ...
            await message.edit(
                f"<i>No response receieved from {message.cmd.title()}</i>\n<b>Reason:</b> <i>{response.reason}</i>"
            )
    ...
    async def converse_with_blackbox(client: Client, message: Message):
        ...
        response = ChatAI.CLIENT.post(
            "https://www.blackbox.ai/api/chat", headers=headers, json=json_data
        )
...
__{response.text}__""",
    ...
    def generate_proof_token(seed, diff, user_agent=str(UserAgent.chrome)):
        ...
        for _ in range(100000):
            ...
            hash_value = sha3_512(
                seed.encode("utf-8") + base64_data.encode("utf-8")
            ).hexdigest()
    ...
    async def converse_with_chatgpt(client: Client, message: Message):
        ...
        if not ChatAI.GPT_CONVERSATION_ID:
            await message.edit(
                "<i>I need you to set a conversation ID first, you can find it in the in the URL in ChatGPT chat page<i>"
            )
            return

        cookies = {"__Secure-next-auth.session-token": CHATGPT_BEARER_KEY}
        ...
        if message.reply_to_text:
            if message.reply_to_text.photo or message.reply_to_text.sticker:
                ...
                if response.json().get("status") != "success":
                    ChatAI.LOG.error(response.text)
                    await message.edit(
                        f"<i>Cannot upload media to ChatGPT</i>\n<b>Reason: </b><i>{response.reason_phrase} (Details in the logs)</i>"
                    )
                    return
                ...
                if not response.is_success:
                    ChatAI.LOG.error(response.text)
                    await message.edit(
                        f"<i>Cannot sign media file in ChatGPT</i>\n<b>Reason: </b><i>{response.reason_phrase} (Details in the logs)</i>"
                    )
                    return
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise ConnectionAbortedError(
                    f"Connection to requirements API received {response.status_code}. Check logs for details..."
                )

            headers["openai-sentinel-chat-requirements-token"] = response.json().get(
                "token"
            )
            ...
            headers["openai-sentinel-turnstile-token"] = (
                response.json().get("turnstile", {}).get("dx")
            )
        except Exception as e:
            await message.edit(
                f"<i>Token cannot be fetched</i>\n<b>Reason: </b><i>{e}</i>"
            )
            return

        headers["openai-sentinel-proof-token"] = ChatAI.generate_proof_token(
            seed, difficulty
        )
                            ...
                            "parts": [
                                {
                                    ...
                                    "width": message.reply_to_text.photo.sizes[
                                        -1
                                    ].width,
                                    "height": message.reply_to_text.photo.sizes[
                                        -1
                                    ].height,
                                },
                                message.raw_args,
                            ...
                            "attachments": [
                                {
                                    ...
                                    "name": message.reply_to_text.file.name
                                    or "photo.jpg",
        ...
        try:
            ...
            await message.edit(
                f"<i>Connection to ChatGPT failed<i>\n<b>Reason:</b> <i>{e}</i>"
            )
            return
        ...
        if response.status_code != 200:
            ChatAI.LOG.error(response.text)
            await message.edit(
                f"<i>Connection to ChatGPT backend api received {response.status_code}. Check logs for details...</i>"
            )
            return
        ...
        if not response:
            ChatAI.LOG.error(
                f"Received and processed data from ChatGPT:\n\n {response}"
            )
            await message.edit(
                "<i>There seems to be a problem with the response from ChatGPT. Check logs for details...</i>"
            )
            return

        response = "".join(
            json.loads(response).get("message").get("content").get("parts")
        )
        await message.edit(
            f"""⏺ **Me: **__{message.raw_args}__
...
Query: {message.raw_args}"""
        ...
        if response.images:
            for image in response.images:
                image_response = get(
                    image.url, allow_redirects=True, cookies=gem_client.cookies
                )
...
{response_text}""",
        ...
        if image_list:
            await message.respond(
                file=image_list, message="<i>Generated by GeminiAI</i>"
            )
    ...
    async def set_gemini_proxy(client: Client, message: Message):
        ...
        if not message.raw_args.count(" ") > 1:
            await message.edit(
                "<i>You need to enter a valid proxy configuration. Example: proxy_type proxy_host proxy_server</i>"
            )
            return
        ...
        try:
            ChatAI.CLIENT = httpx.Client(
                follow_redirects=True, proxies=f"{proxy_type}://{host}:{port}"
            )
        except Exception as e:
            await message.edit(f"<i>{e}</i>")
            return
    ...
    async def get_gemini_proxy(client: Client, message: Message):
        if ChatAI.CLIENT.proxy:
            await message.edit(
                f"<i>{ChatAI.CLIENT.proxy[0]} {ChatAI.CLIENT.proxy[1]} {ChatAI.CLIENT.proxy[2]}</i>"
            )
        else:
    ...
    async def enable_message_listener(client: Client, message: Message):
        ...
        if (
            message.raw_args.lower() != "chatgpt"
            and message.raw_args.lower() != "gemini"
        ):
            await message.edit(
                "<i>Only Gemini and ChatGPT is supported with this feature</i>"
            )
            return

        await message.edit(
            f"<i>Conversation mode with AI is now enabled and set to {message.raw_args.title()}</i>"
        )
    ...
    async def listen_for_message(client: Client, message: Message):
        ...
        if ChatAI.MODE == "chatgpt":
            await ChatAI.converse_with_chatgpt(client, message)

        elif ChatAI.MODE == "gemini":
            await ChatAI.converse_with_gemini_via_cookies(client, message)
