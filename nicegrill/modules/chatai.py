...
__{response}__""",
    ...
    async def converse_with_blackbox(message: Message, client: Client):
        ...
        response = await asyncio.to_thread(ChatAI.CLIENT.post, 'https://www.blackbox.ai/api/chat', headers=headers, json=json_data)
...
__{response.text}__""",
    ...
    async def set_chat_id(message: Message, client: Client):
        ...
        ChatAI.GPT_CONVERSATION_ID = message.args
        settings.set_gpt_chat(message.args)
...
{response_text}""",
    ...
    @run(command="mode")
    async def enable_message_listener(message: Message, client: Client):
        if not message.args:
            ChatAI.MODE = None
            await message.edit("<i>Chat mode is disabled</i>")
            return
        
        if message.args.lower() != "chatgpt" and message.args.lower() != "gemini":
            await message.edit("<i>Only Gemini and ChatGPT is supported with this feature</i>")
            return

        await message.edit(f"<i>Conversation mode with AI is now enabled and set to {message.args.title()}</i>")
        ChatAI.MODE = message.args.lower()

@event_watcher(incoming=False)
async def listen_for_message(message: Message, client: Client):
    if not ChatAI.MODE or message.raw_text.startswith(prefix + "mode"):
        return

    message.args = message.raw_text
