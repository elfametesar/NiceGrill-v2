...
from nicegrill.utils import get_user, humanize
from elfrien.types.objects import Message
from PIL.Image import Image as ImageType
from elfrien.client import Client
from nicegrill import on, startup
from elfrien.types import objects
...
class Quote:
    ...
    async def get_entity_data(entities: list):
        ...
        for entity in entities:
            if isinstance(entity, objects.TextEntityTypeBold):
                entity_type = "bold"
            elif isinstance(entity, objects.TextEntityTypeItalic):
                entity_type = "italic"
            elif isinstance(entity, objects.TextEntityTypeCode) or isinstance(
                entity, objects.TextEntityTypePre
            ):
                entity_type = "mono"
            elif (
                isinstance(entity, objects.TextEntityTypeUrl)
                or isinstance(entity, objects.TextEntityTypeMention)
                or isinstance(entity, objects.TextEntityTypeMentionName)
                or isinstance(entity, objects.TextEntityTypeTextUrl)
            ):
                entity_type = "url"
            elif isinstance(entity, objects.TextEntityTypeUnderline):
    ...
    async def draw_reply_bar(message: Message):
        ...
        name_image = await Quote.draw_text(
            text=message.sender.name, font=Quote.FONT_TITLE
        )
    ...
    async def draw_link_preview(webpage_data: objects.WebPage):
    ...
    async def draw_media(message: Message, text_image: ImageType = None):
        if message.is_media_type and not message.page:
            thumb_buffer = BytesIO()

            try:
                thumb_buffer = await message.download(thumb=1)
            except:
                thumb_buffer = await message.download(thumb=0)
        ...
        for message in message_list:
            ...
            if is_titled:
                title = await Quote.shorten_text(
                    message.sender.name, font=Quote.FONT_TITLE
                )
    ...
    async def draw_text(
        text: str,
        entities: list[objects.TextEntityType] = [],
        font: FontType = None,
        color="white",
        x_offset: int = 0,
    ):
    ...
    @on(pattern="q")
    async def quote_replied_message(client: Client, message: Message):
        ...
        message_count = 1
        if message.raw_args and message.raw_args.isdigit():
            message_count = int(message.raw_args)
        ...
        async for message_object in client.iter_messages(
            entity=message.chat,
            limit=message_count,
            from_message=message.reply_to_text.id,
        ):
            i += 1
        ...
        await message.respond(files=sticker_buffer)

    @on(pattern="fq")
    async def fake_quote(client: Client, message: Message):
        ...
        arguments = message.raw_args.split()
        if len(arguments) < 2:
            await message.edit(
    ...
    @on(pattern="setcolor")
    async def set_message_color(client: Client, message: Message):
        if not message.raw_args:
        ...
        if len(arguments) > 3 and "".join(arguments).isdigit():
            ...
            color = (r, g, b, a)
        elif message.raw_args.startswith("#"):
            color = message.raw_args
        else:
            color = message.raw_args
...
def load_from_database():
    Quote.MESSAGE_COLOR = quote.get_message_color()
