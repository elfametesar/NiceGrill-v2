...
def wrapper(func, name):
    async def inner_content(client: Client, message: Message, **kwargs):
        try:
            if message.chat_id not in bad_chat_list or (
                message.chat_id in bad_chat_list
                and message.cmd in ["blacklist", "whitelist"]
            ):
...
def on(
    ...
    def inner_function(func):
        ...
        update_help_data(function=func, command=pattern)
    ...
    def inner_function(func):
        nicegrill.client.add_event_handler(
            event=ServiceMessage(
                chats=chats, blacklist_chats=blacklist_chats, func=condition
            ),
            callback=func,
        )
        return func
